	...
	if Global.planting == true:
		if not plant_growing:
			...
			if plant_type == "tree":
				...
				$TreeTimer.wait_time = Global.plant_data.get("growtime")
				$TreeTimer.start()
...
func _on_tree_timer_timeout() -> void:
	if tree_grown == false:
		...
		if growing_plant.frame != 1:
			...
			$FruitTimer.wait_time = Global.plant_data.get("growtime")
			$FruitTimer.start()
			$Plant.play("apple_fruit_growing")
